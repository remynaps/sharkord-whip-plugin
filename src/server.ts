...
const onLoad = async (ctx: PluginContext) => {
  ...
  const settings = await ctx.settings.register([
    ...
    {
      key: "expose_stats",
      name: "Expose Stats Endpoint",
      description:
        "Enables GET /whip/stats/:channelId. This is required for the stream stats UI. Only enable if your WHIP port is behind a reverse proxy.",
      type: "boolean",
      defaultValue: false,
    },
  ] as const);
