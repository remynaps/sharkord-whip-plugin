...
import {
  ...
  type PluginContext, type UnloadPluginContext
...
class WhipServerManager {
  ...
  public async start(ctx: PluginContext, settings: any, rtpMinPort: number, rtpMaxPort: number): Promise<string> {
    if (this.canStart()) {
...
const onLoad = async (ctx: PluginContext) => {
  ...
  registerCommand(
    ...
    async () => {
      return await whipServerManager.start(ctx, settings, rtpMinPort, rtpMaxPort);
    },
  );
